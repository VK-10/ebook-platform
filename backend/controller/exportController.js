  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      if (token.type === "heading_open") {
        ...
        // The list item content may be nested; find the inline token
        ...
        if (inlineToken && inlineToken.children) {
          ...
          paragraphs.push(
            new Paragraph({
              ...
              indent: { left: 720 }, // 0.5" approximate
            })
          );
        }
      } else if (token.type === "blockquote_open") {
        // collect inline inside blockquote
        let inlineToken = null;
        for (let j = i + 1; j < tokens.length; j++) {
          if (tokens[j].type === "inline") {
            inlineToken = tokens[j];
...
// process inline content (bold, italic, text, code)
    ...
    if (child.type === "strong_open") {
      ...
      textRuns.push(
        new TextRun({
          text: child.content,
          font: "Courier New",
          size: Math.max(10, DOCX_STYLES.sizes.body * 2 - 2),
        })
      );
    ...
    if (Array.isArray(book.chapters)) {
      book.chapters.forEach((chapter, index) => {
        try {
          ...
          // chapter title (use chapter.title if present)
          sections.push(
            new Paragraph({
              children: [
                new TextRun({
...
const renderInlineTokens = (doc, tokens, options = {}) => {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (t.type === "text") {
      ...
      flushBuffer();
      doc.font("Courier").text(t.content, {
        ...baseOptions,
        continued: true,
      });
...
const exportAsPDF = async (req, res) => {
  try {
    ...
    doc.font(TYPOGRAPHY.fonts.sansBold).fontSize(TYPOGRAPHY.sizes.title).fillColor(TYPOGRAPHY.colors.heading).text(book.title, {
      align: "center",
    });
    ...
    if (book.subtitle && book.subtitle.trim()) {
      doc.font(TYPOGRAPHY.fonts.sans).fontSize(TYPOGRAPHY.sizes.h2).fillColor(TYPOGRAPHY.colors.text).text(book.subtitle, {
        align: "center",
      });
    }

    doc.font(TYPOGRAPHY.fonts.sans).fontSize(TYPOGRAPHY.sizes.author).fillColor(TYPOGRAPHY.colors.text).text(`by ${book.author}`, {
      align: "center",
    });
    ...
    if (book.chapters && book.chapters.length > 0) {
      book.chapters.forEach((chapter, index) => {
        try {
          ...
          doc.font(TYPOGRAPHY.fonts.sansBold).fontSize(TYPOGRAPHY.sizes.chapterTitle).fillColor(TYPOGRAPHY.colors.heading).text(chapter.title || `Chapter ${index + 1}`, {
            align: "left",
          });
