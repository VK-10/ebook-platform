...
const SignupPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // save token so axios interceptor can send Authorization header
      localStorage.setItem("token", token);

      // fetch profile (axiosInstance will include Authorization header)
      const profileResponse = await axiosInstance.get(API_PATHS.AUTH.GET_PROFILE);

      // call login from context with user data and token (adjust if your context expects different shape)
      login(profileResponse.data, token);

      toast.success("Account created successfully!");
      navigate("/dashboard");
    } catch (error) {
      // clear any possible stale token
